  ...
  <section class="container">
    <div class="bookmarksMainContainer" :style="'background-color:'+mainStore.currentService.colors.tabBgColor">
      ...
      <ul class="bookmarksContainer hideMob pr" :style="'color:'+mainStore.currentService.colors.tabColorText">
        <div class="tabItemDots hover" @click="isPanelTabOpen=!isPanelTabOpen" v-if="cList.length && (!isPublic || isRedactor)">
          ...
        </div>
        <p v-if="isPublic" class="copyBtn hover cy" @click="copyPublicNotesToLocal">copy this catalog to my list</p>
        <li
          v-for="item in cTabs"
          :key="item"
        ...
        <button class="myBtn px-3 cb mt20 fc" @click="addCatalog" v-if="!cList.length">add first catalog</button>
        <li class="servContainer" v-if="isPanelTabOpen && (!isPublic || isRedactor)">
          ...
          <input
            ...
            required
          >
          <button class="myBtn px-3 cb mt10" @click="update">save</button>
        </li>
      ...
      <ul class="bookmarksSubContainer hideMob pr" v-if="cTabItem" >
        ...
        <button class="myBtn px-3 cb mt20 fc" @click="addNote" v-if="!cSubTabList.length">add first note</button>
        <div class="tabItemDots hover" @click="isPanelSubTabOpen=!isPanelSubTabOpen" v-if="cSubTabList.length">
          ...
        </div>
  ...
  export default {
    ...
    computed:{
      ...
      cList(){
        if(this.list){
         ...
         else return this.mainStore?.currentInfo?.notesList||[]
    ...
    methods:{
      copyPublicNotesToLocal(){
        ...
        this.mainStore.current.infoName = 'beloeInfo'
        updateFB({path:'notesList', data:this.mainStore.beloeInfo.notesList});
        this.mainStore.showMsg( { text:'copied successfully', type:'success'});
      },
      update(){
